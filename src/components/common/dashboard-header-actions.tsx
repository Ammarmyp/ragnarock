...
export function DashboardHeaderActions({ user }: DashboardHeaderActionsProps) {
  ...
  const projectId = pathname?.match(/^\/dashboard\/projects\/([^/]+)/)?.[1] ?? null;
  const { data: project, isPending: projectPending } = useProject(projectId ?? "", { enabled: Boolean(projectId) });
  const { data: role, isPending: rolePending } = useProjectRole(projectId ?? "", { enabled: Boolean(projectId) });
      ...
      {projectId ? (
        <div className="hidden min-w-0 flex-col items-end leading-tight sm:flex">
          {projectPending ? (
            ...
            <span className="max-w-2xs truncate text-sm font-semibold">{project?.name ?? "Project"}</span>
          )}
          {rolePending ? (
            <Skeleton className="mt-1 h-3.5 w-20" />
          ) : (
            <span className="text-muted-foreground max-w-2xs truncate text-xs">
              Role: {role?.role ? role.role[0]!.toUpperCase() + role.role.slice(1) : "—"}
            </span>
          )}
        </div>
      ) : null}
