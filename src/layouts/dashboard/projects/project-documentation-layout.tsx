...
import { Bot, Eye, FileText, Loader2 } from "lucide-react";
...
import { specToMarkdown } from "@/lib/spec-to-markdown";
import { cn } from "@/lib/utils";
import type { ProjectAiChatSessionWithProgress, ProjectSpecification } from "@/api/projects.api";
...
function SrsCard({
  ...
  const markdown = useMemo(
    () => (data.spec ? specToMarkdown(data.spec.payload) : null),
    [data.spec],
  );
      ...
      <CardFooter className="flex flex-wrap gap-2 border-t border-border/50 bg-muted/20 pt-3">
        <Button size="sm" asChild className="shadow-none">
          <Link href={detailHref}>
            ...
            {isComplete ? "View full SRS" : "Continue in Requirements"}
          </Link>
        </Button>
