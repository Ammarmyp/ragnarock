You are a senior frontend engineer. Generate a production-ready React form using **TanStack Form**, **Zod**, and **shadcn/ui**.
...
* Use `shadcn/ui` components (`Field`, `Input`, `Textarea`, etc.)
...
* Use `defaultValues`
* Use `validators` (prefer `onSubmit`, optionally `onChange` or `onBlur`)
* Implement `onSubmit` handler

---

### 3. Form Structure (STRICT)
...
### 4. Field Pattern (NON-NEGOTIABLE)

Every field MUST follow this exact structure:
...
* Use `onValueChange={field.handleChange}`
...
* Add:
  ...
  * `aria-invalid` on input component
...
* Every input MUST have:
  ...
  * `name`
  * `aria-invalid`
* Every label MUST use `htmlFor`
* Use semantic components (`Field`, `FieldLabel`, etc.)

---

### 9. Array Fields (If Needed)
...
"Create a user registration form with name, email, password, role (select), and terms (checkbox)"
...
* Do NOT invent patterns outside this standard
* Follow the field pattern EXACTLY and the official shadcn integration guide also the following attached https://ui.shadcn.com/docs/forms/tanstack-form page
* For any UI use shadcn components as much as possible

## Tanstack form doc - https://ui.shadcn.com/docs/forms/tanstack-form
---
title: TanStack Form
